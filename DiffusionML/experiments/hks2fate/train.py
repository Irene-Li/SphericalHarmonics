"""
Training script: predict cell fate markers from HKS features.

Uses DiffusionNet to learn a per-vertex mapping from Heat Kernel Signatures
to cell fate marker fields on organoid surfaces.

Usage:
    python ML/experiments/hks2fate/train.py --data_path Data/20260224 --epochs 200
"""
...
def train_one_epoch(model, dataset, indices, optimizer, device, epoch):
    ...
    for idx in pbar:
        ...
        optimizer.zero_grad()
        fate_pred = model(hks, mass, L, evals, evecs, gradX, gradY)
        loss = area_weighted_mse(fate_pred, fate, mass)
        loss.backward()
        optimizer.step()
...
def main():
    parser = argparse.ArgumentParser(
        description="Train DiffusionNet: HKS -> cell fate markers")
    parser.add_argument('--data_path', type=str, default='Data/small_meshes',
                        help='Path to small_meshes directory')
    parser.add_argument('--output_dir', type=str, default='DiffusionML/experiments/hks2fate/outputs',
    ...
    print(f"\nTask: HKS ({n_hks} channels) -> Fate markers ({n_fates} channels)")
