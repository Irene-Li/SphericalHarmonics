...
if __name__ == "__main__":
    ...
    meshes = {
        'day1p5': ['nnorg_linked_annotated'],
        'day2': ['nnorg_linked_annotated'],
        'day2p5': ['nnorg_linked_annotated'],
        'day3': ['nnorg_linked_annotated'],
        'day3p5': ['nnorg_linked_annotated'],
        'day4': ['nnorg_linked_annotated'],
        'day4p5': ['nnorg_linked_annotated'],
        'day4p5-more': ['nnorg_linked_annotated'],
    }
    ...
    for timepoint in timepoints:
        ...
        for well_name in wells[timepoint]:
            ...
            path = f"Data/20250714/fractal_output/{timepoint}/{zarr_name}/{well_name[0]}/{well_name[1:]}/{round_name}/"
            labels = np.load(path + 'good_labels.npy').astype('int')
            mesh_name = meshes[timepoint][0]
            print(path) 
            for label in tqdm(labels):
                mesh_path = f"Data/20250714/fractal_output/{timepoint}/{zarr_name}/{well_name[0]}/{well_name[1:]}/{round_name}/meshes/{mesh_name}/{label}.vtp"
                if not os.path.exists(mesh_path):
                    print(f"Mesh file does not exist: {mesh_path}")
                    continue
                save_path = f"Data/20250714/fractal_output/{timepoint}/{zarr_name}/{well_name[0]}/{well_name[1:]}/{round_name}/fm_data"
